...
public void actionPerformed(ActionEvent e) {
	...
	if (usuario.length() > 0 && contrasena.length() > 0){
		if(validarUsuario(usuario,contrasena)){
			...
			try {
				if (rs.getBoolean(4)){
					InterfazAdministrador adm = new InterfazAdministrador();
					adm.initInterfazAdministrador();
					this.dispose();
				}
				else {
...
public boolean validarUsuario(String usr, String pswd){
	try {
		...
		System.out.println("usuario:"+usr+"Contraseña:"+pswd);
		rs= con.consulta("SELECT * FROM usuarios WHERE usuario= '"+usr+"' AND contraseña='"+pswd+"'");
		if (rs.first()){
			return true;
